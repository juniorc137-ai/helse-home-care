...
describe("indicatorVisuals (mapeamento clínico -> anel do health card)", () => {
  it("Braden fixture (escore 10, Risco alto) mapeia para cor de perigo e severidade alta", () => {
    const visual = bradenToVisual({ type: "braden", scores: BRADEN_FIXTURE_HIGH_RISK }, lightTokens);
    expect(visual.value).toBe("10");
    expect(visual.sublabel).toBe("Risco alto");
    expect(visual.color).toBe(lightTokens.statusDanger);
  ...
  it("Braden sem risco mapeia para cor segura e severidade baixa", () => {
    ...
    expect(visual.color).toBe(lightTokens.statusOk);
    expect(visual.percent).toBeLessThan(20);
  });

  it("TEC fixture (3,5s, Alterado) mapeia para cor de perigo", () => {
    const visual = tecToVisual({ type: "tec", seconds: TEC_FIXTURE_ALTERED_SECONDS, hasContextualFactor: false }, lightTokens);
  ...
  it("Morse fixture (escore 50, Risco elevado) mapeia para cor de perigo", () => {
    const visual = morseToVisual({ type: "morse", scores: MORSE_FIXTURE_HIGH_RISK }, lightTokens);
  ...
  it("percentuais sempre ficam no intervalo 0-100", () => {
    expect(bradenToVisual({ type: "braden", scores: BRADEN_FIXTURE_HIGH_RISK }, lightTokens).percent).toBeLessThanOrEqual(100);
    expect(morseToVisual({ type: "morse", scores: MORSE_FIXTURE_HIGH_RISK }, lightTokens).percent).toBeLessThanOrEqual(100);
    expect(tecToVisual({ type: "tec", seconds: 10, hasContextualFactor: false }, lightTokens).percent).toBeLessThanOrEqual(100);
  });
});
